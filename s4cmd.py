...
def progress(msg, *args):
  ...
  # Used to clear the trailing part of the previous message which
  # won't be overwritten by the new message.
  trailing_whitespace_length = 0
  if progress.prev_message and len(progress.prev_message) > len(text):
    trailing_whitespace_length = len(progress.prev_message) - len(text)

  sys.stderr.write(text + ' ' * trailing_whitespace_length + '\r')
